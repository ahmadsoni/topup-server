  ...
  checkout: async (req, res) => {
    try {
      ...
      const res_voucher = await Voucher.findOne({ _id: voucher })
        ...
        .populate('category')
        .populate('user');
      if (!res_voucher) return res.status(404).json({ message: 'voucher game tidak di temukan' });
      ...
      if (!res_bank) return res.status(404).json({ message: 'payment tidak di temukan' });
